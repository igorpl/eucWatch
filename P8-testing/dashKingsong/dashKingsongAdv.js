...
face[0] = {
	...
	init: function(){
		...
		this.g.drawString(md[euc.dash.opt.ride.mode],60-(this.g.stringWidth(md[euc.dash.opt.ride.mode])/2),50); 
		this.g.flip();
		//calibrate
	...
	switch (e) {
      case 5:case 12: //tap event
		if ( x<=120 && y<=100 ) { //ride mode
			euc.dash.opt.ride.mode++; if (2<euc.dash.opt.ride.mode)euc.dash.opt.ride.mode=0;
			let m=["HARD","MED","SOFT"]
			face[0].btn("MODE",18,60,20,4,0,0,119,97,m[euc.dash.opt.ride.mode],30,60,50);
			...
			buzzer.nav([30,50,30]);		
			if (euc.dash.opt.lock.pass.length>=4) face.go("dashKingsongAdvPass",5);
			else {
				euc.wri("getPass");
