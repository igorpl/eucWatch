...
touchHandler[0]=function(e,x,y){
        ...
        if (!face[0].setE){//select page
			...
			if (120<=x) { //up
                if (face[0].setEb=="one") 
                    ...
                    if (euc.dash.alrt.spd.tilt.val<75) euc.dash.alrt.spd.tilt.val++;
                    else face[0].ntfy("THE LIMIT IS 75KpH","",20,13,1);
                    } 
            }else {  //dn
                if (face[0].setEb=="one") {
					if (5<euc.dash.alrt.spd.one.val) euc.dash.alrt.spd.one.val--;
                }else if (face[0].setEb=="two"){
					if (euc.dash.alrt.spd.one.en){
					...
					else face[0].ntfy("AL2 N LIMIT IS 10KpH","",20,13,1);
                }else if (face[0].setEb=="tilt"){
            ...
            buzzer.nav([30,50,30]);
			face[0].btn(1,euc.dash.alrt.spd[face[0].setEb].val,100,126,60,12,1,60,40,180,160);
		}
		this.timeout();
		break;
	...
	case 2: //slide up event
		if (!ew.def.noBriSw&&y>200&&x<50) { //toggles full/current brightness on a left down corner swipe up. 
   ...
   case 12: //hold event
			if (x<=120&&y<100) { //alarm 1
               // if (euc.dash.alrt.spd.two.en){
                if (euc.dash.alrt.spd.one.val>=euc.dash.alrt.spd.two.val) 
				euc.dash.alrt.spd.one.val=euc.dash.alrt.spd.two.val-1;
				face[0].set("one","ALARM 1");
				buzzer.nav([30,50,30]);
