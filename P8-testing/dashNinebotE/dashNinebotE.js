...
face[0] = {
	...
	init: function(){
		...
		this.g.flip();
		//
        this.btn(euc.dash.auto.onD.lock,"AUTO",18,60,20,13,1,0,0,119,97,"LOCK",25,60,55);
...
touchHandler[0]=function(e,x,y){ 
	...
	case 5: //tap event
		if (face[0].set) { 
			if ( 100 < y ) {
			  euc.wri(1,30+euc.dash.opt.ride.mode);
              w.gfx.setColor(0,0);
			...
			if ( x<=120 && y<100 ) { //auto lock
				...
				face[0].btn(euc.dash.auto.onD.lock,"AUTO",18,60,20,13,1,0,0,119,97,"LOCK",25,60,55);
				face[0].ntfy("DISCONNECT -> LOCK","AUTO LOCK DISABLED",18,(euc.dash.auto.onD.lock)?13:1,euc.dash.auto.onD.lock);
				buzzer.nav([30,50,30]);
                ...
                euc.wri(1,25+euc.dash.opt.lght.HL);
				buzzer.nav([30,50,30]);	
	...
	case 1: //slide down event
	    if (face[0].set) {
			euc.wri(1,1,30+euc.dash.opt.ride.mode);
			setTimeout(()=>{euc.is.busy=0;euc.wri(1,1);},500);
        }
		//face.go("clock",0);
	...
	case 12: //long press event
		if (face[0].set) { 
			...
			euc.dash.auto.onD.lock=1-euc.dash.auto.onD.lock;
            face[0].btn(euc.dash.auto.onD.lock,"AUTO",18,60,20,13,1,0,0,119,97,"LOCK",25,60,55);
            face[0].ntfy("DISCONNECT -> LOCK","AUTO LOCK DISABLED",18,1,euc.dash.auto.onD.lock);
			buzzer.nav([30,50,30]);
		}else if ( 120<=x && y<=100 ) { //watch alerts
