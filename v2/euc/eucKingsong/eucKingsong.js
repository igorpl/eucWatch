				...
				if (n === "proxy") {
					...
					if (euc.gatt && euc.gatt.connected) {
						...
						Promise.resolve().then(function() {
							...
							if (euc.dash.auto.onD.led) return c.writeValue(euc.cmd("setLedRideOnOff", euc.dash.auto.onD.led - 1));
						}).then(function() {
							if (euc.dash.auto.onD.lift) return c.writeValue(euc.cmd("setLiftOnOff", 2 - euc.dash.auto.onD.lift));
						}).then(function() {
							if (euc.dash.auto.onD.talk) return c.writeValue(euc.cmd("setVoiceOnOff", 2 - euc.dash.auto.onD.talk));
						}).then(function() {
							euc.is.run = 0;
